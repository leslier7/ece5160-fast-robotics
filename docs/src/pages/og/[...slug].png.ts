...
export async function getStaticPaths() {
  ...
  const pages = [
    ...
    { slug: 'labs', title: 'Labs', subtitle: 'Lab Reports' },
  ];
  ...
  // Generate paths for labs
  const labPaths = labs.map(item => ({
    params: { slug: `labs/${item.id.replace(/\.[^/.]+$/, "")}` },
    props: { 
