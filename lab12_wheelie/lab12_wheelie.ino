...
// ========================
const int motorB_In1 = 14;   // Left  — normal polarity
const int motorB_In2 = 13;
const int motorA_In1 = 15;   // Right — INVERTED
...
void stop_motors() {
    analogWrite(motorA_In1, 0); analogWrite(motorA_In2, 0);
    analogWrite(motorB_In1, 0); analogWrite(motorB_In2, 0);
}

void brake_motors() {
    analogWrite(motorB_In1, 255); analogWrite(motorB_In2, 255);
    analogWrite(motorA_In1, 255); analogWrite(motorA_In2, 255);
}

// Positive speed = forward (front wheels drive, rear lifts for wheelie)
void drive_forward(int speed) {
    speed = constrain(speed, 0, 255);
    if (speed == 0) { stop_motors(); return; }
    int pwm_l = map(speed, 1, 255, DEADBAND_L, 255);
    int pwm_r = constrain((int)(pwm_l * (float)DEADBAND_R / (float)DEADBAND_L), 0, 255);
    analogWrite(motorB_In1, pwm_l); analogWrite(motorB_In2, 0);
    analogWrite(motorA_In1, 0);     analogWrite(motorA_In2, pwm_r);
}
...
void drive_backward(int speed) {
    ...
    int pwm_l = map(speed, 1, 255, DEADBAND_L, 255);
    int pwm_r = constrain((int)(pwm_l * (float)DEADBAND_R / (float)DEADBAND_L), 0, 255);
    analogWrite(motorB_In1, 0);     analogWrite(motorB_In2, pwm_l);
    analogWrite(motorA_In1, pwm_r); analogWrite(motorA_In2, 0);
}

// pid_out > 0 → forward (drives wheels to push car toward upright)
// pid_out < 0 → backward (corrects over-tilt)
void drive_pid_output(float pid_out) {
    int speed = (int)abs(pid_out);
...
void setup() {
    ...
    pinMode(motorB_In1, OUTPUT); pinMode(motorB_In2, OUTPUT);
    stop_motors();
